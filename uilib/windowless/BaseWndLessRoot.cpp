...
#include "utils/ImageEx/ImageEx.h"
...
void BaseWndLessRoot::CalcBkgndGraySale() 
{
	...
	{
		m_bkgnd_gray_scale = skin.GetRoughGrayScale() ;
	}
}
...
void BaseWndLessRoot::DrawBkgnd()
{
	...
	else
	{
		...
		int gradient_height = 20 ;
		int title_begin = m_title_height - gradient_height / 2 ;
		Graphics gph(m_bkgnd_dc) ;
		LinearGradientBrush lgb(Point(0, title_begin), Point(0, title_begin + gradient_height), Color(0, 0, 0, 0), Color(255, 0xf3, 0xfb, 0xff)) ;
		gph.FillRectangle(&lgb, 0, title_begin, window_size.cx, gradient_height) ;

		SolidBrush sb(Color(255, 0xf3, 0xfb, 0xff)) ;
		gph.FillRectangle(&sb, 0, title_begin + gradient_height, window_size.cx, window_size.cy - title_begin + gradient_height) ;
