...
WLAnimationImp::WLAnimationImp(void)
:
m_uImageColCount(1)
, m_uImageRowCount(1)
, m_uPlayElapse(80)
...
BOOL WLAnimationImp::Open(LPCTSTR lpszPath) 
{
	if (GetStyle() & AS_CMBD)
		return FALSE ;
...
BOOL WLAnimationImp::Open(UINT nID) 
{
	if (GetStyle() & AS_CMBD)
		return FALSE ;
...
BOOL WLAnimationImp::Open(Shared_ImageEx &imgAnimation) 
{
	m_imgAnimation = imgAnimation ;
	m_imgAnimation.SetCallBackFunction(boost::bind(&WLAnimationImp::PlayCallbackFucntion, this)) ;
	m_imgAnimation->Stop() ;
	UpdateView() ;

	if (GetStyle() & AS_AUTOFITIMAGE)
	{
...
BOOL WLAnimationImp::Open(LPCTSTR lpszPath, UINT uCol, UINT uRow) 
{
	if (!(GetStyle() & AS_CMBD))
		return FALSE ;
...
BOOL WLAnimationImp::Open(UINT nID, UINT uCol, UINT uRow) 
{
	if (!(GetStyle() & AS_CMBD))
		return FALSE ;
...
BOOL WLAnimationImp::Open(Shared_ImageEx &imgAnimation, UINT uCol, UINT uRow) 
{
	m_imgAnimation = imgAnimation ;
...
void WLAnimationImp::Close() 
{
	m_imgAnimation.Reset() ;
}

UINT WLAnimationImp::SetPlayElapse(UINT uPlayElapse) 
{
...
BOOL WLAnimationImp::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT * pResult) 
{
	BOOL bRes = FALSE ;
	*pResult = 0 ;
...
void WLAnimationImp::OnDraw(HDC hDC, RECT const &rcUpdate) 
{
...
void WLAnimationImp::OnTimer(UINT_PTR nIDEvent) 
{
...
void WLAnimationImp::PlayCallbackFucntion() 
{
	UpdateView() ;
}

template<>
UILIB_EXPORT WLAnimationImpBase *CreateDefaultImp< WLAnimationImpBase >(IWLAnimation * wrapper)
{
	WLAnimationImpBase *ret = new WLAnimationImp();
